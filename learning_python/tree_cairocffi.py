...
def get_color(level, root_color, tip_color):
    ...
    """
    return ((level*1.0/ITERATIONS)*(root_color[0]-tip_color[0])+tip_color[0],(level*1.0/ITERATIONS)*(root_color[1]-tip_color[1])+tip_color[1],(level*1.0/ITERATIONS)*(root_color[2]-tip_color[2])+tip_color[2])
...
def fractal_tree(ctx,			# a cairo context to draw on
    ...
    fractal_tree(ctx, iterations-1, (x, y),  t*r, r, theta + (random.random())*(perturb/(iterations+1))*angle, angle, perturb, root_color, tip_color) 
    fractal_tree(ctx, iterations-1, (x, y),  t*r, r, theta - (random.random())*(perturb/(iterations+1))*angle,angle, perturb, root_color, tip_color)
...
def main():
    ...
    ctx.paint()
    fractal_tree(ctx, ITERATIONS, ROOT, TRUNK_LEN, SCALE, THETA, ANGLE, PERTURB, ROOT_COLOR, LEAF_COLOR)
